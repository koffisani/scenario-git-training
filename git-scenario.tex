...
\end{bash}
Modifier le fichier README.md. Ouvrir le fichier dans un éditeur et ajouter les lignes suivants:
\begin{bash}
...
\end{bash}Ajouter le fichier modifié à l'index (staging erea):
\begin{bash}
git add README.md
\end{bash}
Valider les modifications :
...
\end{bash}
pour ajouter tous les fichiers déjà suivis (et modifiés) et les valider.
...
La fusion de branches consiste à fusionner les modifications effectuées sur deux branches différentes en une seule.
\begin{itemize}
\item Basculer vers la branche master 
\begin{bash}
git checkout master
\end{bash}
\item Fusionner la branche update-readme avec master
\begin{bash}
